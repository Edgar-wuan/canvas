...
(function () {
  ...
  function buildScreens() {
    ...
    const headerBytes = P.packHeader({
      ...
      flags: compressed ? P.FLAG_GZIP : 0,
      totalFrames,
    ...
    let sinceHeader = HEADER_EVERY; // 首屏先发头
    // 正序播放
    for (let i = 0; i < totalFrames; ) {
      if (sinceHeader >= HEADER_EVERY) {
        if (mode === 'bw') pushHeaderScreenBW();
        else pushHeaderScreenColor();
        sinceHeader = 0;
      }
      if (mode === 'bw') {
    ...
    // 正倒放：BW 模式下数据帧倒序再放一遍
    // 包含最后一帧：正序播完帧 N-1 后立即重发 N-1，差最后一张时等 1 屏而非一整轮
    // 接收端收到重复帧会自动忽略（chunks.has 判重），无副作用
    if (mode === 'bw' && totalFrames > 1) {
      for (let j = totalFrames - 1; j >= 0; j--) {
        if (sinceHeader >= HEADER_EVERY) { pushHeaderScreenBW(); sinceHeader = 0; }
        pushDataScreenBW(dataStrs[j]);
        sinceHeader += 1;
      }
    }
    return { screens, totalFrames, dataVer, headerVer, fileCrc, compressed };
  }
  ...
  function start() {
    ...
    try {
      ...
      sendStats.textContent = `共 ${screens.length} 屏 · ${r.totalFrames} 数据帧 · 数据v${r.dataVer} 头v${r.headerVer}${zipTag} · CRC ${r.fileCrc.toString(16).padStart(8, '0')}`;
    } catch (e) {
  ...
  turboBtn.addEventListener('click', () =>
    applyPreset({ mode: 'bw', cs: 1090, ecc: 'M', vm: 25, ms: 150, scale: 8 })
  );
