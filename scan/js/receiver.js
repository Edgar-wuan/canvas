...
(function () {
  ...
  // 接收状态
  let header = null;       // {totalFrames, fileSize, chunkSize, name, fileCrc, txMode: 'bw'|'color'}
  ...
  function loop() {
    ...
    if (!_busy && video.readyState >= 2 && !done) {
      ...
      // 用 header.txMode 优先决定解码策略，避免编码模式错误
      let txMode = header ? header.txMode : null;
      // 用户手动勾选优先；若已收头帧则以头帧标记为准（修正用户误操作）
      let useColor = color;
      if (txMode) useColor = (txMode === 'color');

      let decoded = false;

      if (useColor) {
        // 多色模式：优先 R/G/B 各通道 Otsu 二值化解码
        for (let ch = 0; ch < 3 && !decoded; ch++) {
          const chData = thresholdChannel(img.data, w, h, ch);
          decoded = tryDecode(chData, w, h);
        }
        // 兜底：灰度化+Otsu 可能也能解出单通道
        if (!decoded) {
          const gray = toGray(img.data, w, h);
          decoded = tryDecode(gray, w, h);
        }
        if (!decoded) {
          const bin = thresholdChannel(img.data, w, h, 0);
          decoded = tryDecode(bin, w, h);
        }
      } else {
        // 黑白模式：只走灰度化 + Otsu，不试 RGB 通道（避免误判）
        // 1. 灰度化 BW 解码
        const gray = toGray(img.data, w, h);
        decoded = tryDecode(gray, w, h);
  ...
  function handleBytes(bytes) {
    ...
    if (fr.kind === 'header') {
      if (!header || header._partial) {
        ...
        showFileCard();
        updateProgress();
        const recvColor = (fr.flags & P.FLAG_COLOR) !== 0;
        header.txMode = recvColor ? 'color' : 'bw';
        const userColor = colorModeRecv.checked;
        let modeMsg = '';
        if (userColor !== recvColor) {
          setStatus(`模式不匹配！发送端=${recvColor ? '多色' : '黑白'}，请${recvColor ? '勾选' : '取消勾选'}多色模式后重试`, false);
        } else {
          setStatus('已扫到文件头，可以开始传输');
        }
        recvStats.textContent = `已扫到文件头：${fr.name} · ${fr.totalFrames} 块 · ${formatSize(fr.fileSize)} · 编码=${recvColor ? '多色' : '黑白'}${modeMsg}`;
        if (chunks.size >= header.totalFrames) finish();
      const recvColor = (fr.flags & P.FLAG_COLOR) !== 0;
      header.txMode = recvColor ? 'color' : 'bw';
      const userColor = colorModeRecv.checked;
      if (userColor !== recvColor) {
        setStatus(`模式不匹配！发送端=${recvColor ? '多色' : '黑白'}，请${recvColor ? '勾选' : '取消勾选'}多色模式`, false);
      }
  ...
  function showFileCard() {
    ...
    const encLabel = (header.textEncoding === 1) ? 'GBK' : 'UTF-8';
    fileCrc.textContent = (header.fileCrc ? '0x' + header.fileCrc.toString(16).padStart(8, '0') : '—') + ' · 编码 ' + encLabel;
  }
